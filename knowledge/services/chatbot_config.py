...
def build_system_prompt(cfg: ResolvedConfig) -> str:
    ...
    return (
        ...
        f"- Do NOT start your replies with a greeting like \"أهلاً وسهلاً\", \"مرحباً\" or "
        f"\"Hello\". Greet back ONLY when the customer actually greets you first; for a normal "
        f"question, answer the question directly — no opening pleasantries every time.\n"
...
def build_no_data_prompt(question: str, cfg: ResolvedConfig) -> str:
    ...
    return (
        ...
        f"You have NO information to answer this. Reply briefly, warmly, and like a real "
        ...
        f"- For ANY actual question (general knowledge, other companies, tech, world facts, "
        f"opinions, or anything you weren't given data about): you MUST decline. Say politely "
        f"that it's outside what you can help with here, and invite them to ask about us.\n"
        f"ABSOLUTE RULE: NEVER answer the question from your own/general knowledge. Do NOT "
        f"provide any fact, product, price, name, definition, or detail. Declining is the only "
        f"correct response when you have no data"
        + (
            ". And do NOT invent or guess a company/organization name — just say you're an "
